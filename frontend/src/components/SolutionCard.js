...
const SolutionCard = ({title,className,id,width,solutionHref}) => {
    ...
    const settingStorage = useCallback((a)=>setStorage(a))
    ...
    return (
        <>
           {storage!=undefined &&
           <>
           <General.BgImage className={`inline-block bg-center bg-cover bg-no-repeat bg-gray-300 rounded-3xl text-white ${className}`} src={storage.img_sc} width={width}>
                <div className='w-full h-full relative opacity-0 transition-all duration-300 hover:opacity-100'>
                    <div className='flex flex-col items-center justify-end h-1/2 w-full absolute bottom-0 rounded-b-3xl bg-gradient-to-t from-black to-transparent '>
                        <h4 className='openBold text-xl text-center mb-10'>{title}</h4>
                        {solutionHref!=undefined?
                        <General.Trigger
