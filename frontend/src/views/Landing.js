...
const Landing = () => {
    ...
    const {getSolutions,getSolution,charged,setCharged,getCategoriesBySolution} = usePageContext()

    const [solutions,setSolutions] = useState()

    const setData = async() =>{
        let s = Object.values(await getSolutions())
        for(let a of s){
            a.categories = await getCategoriesBySolution(a.id)
            for(let b of a.categories){
            b.route = a.name_s+'/'+b.id
            }
        }
        setSolutions(s)
        setCharged(true)
        ...
        return (
            ...
            <General.Header mode="ligth" items={solutions}/>
                <main className='w-full relative z-20 text-white px-4 lg:px-16 xl:px-28 2xl:px-48'>
                    ...
                    <General.Cloner items={Object.values(solutions)} rprops={[['key','id'],['children','tittle_s'],['href','route'],['src','img_s']]}>
                        <SolutionMiniCard src={BASE_URL + '/assets/img/Categories/g486.png'}/>
                    </General.Cloner>
                    </ul>
                    <General.Cloner items={Object.values(solutions)} rprops={[['key','id'],['title','tittle_s'],['id','id'],['href','route'],['element','categories']]}>
                        <SolutionsCarousel />
                    </General.Cloner>
                </main>
            </General.BgImage>
